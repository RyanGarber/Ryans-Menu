...
Ryan.Player.Self = function()
    return Ryan.Player.CreateOrUpdate(players.user())
end

-- Get a player by its index.
Ryan.Player.ById = function(id)
    return Ryan.Player.CreateOrUpdate(id)
end
...
Ryan.Player.ByPedId = function(ped_id)
    for _, player in pairs(Ryan.Player.List(true, true, true)) do
        if player.ped_id == ped_id then
            return Ryan.Player.CreateOrUpdate(player.id)
        end
    end
    return nil
end
...
Ryan.Player.ByName = function(name)
    ...
    return fallback_player ~= nil and Ryan.Player.CreateOrUpdate(fallback_player.id) or nil
end

-- List all players in the session.
Ryan.Player.List = function(include_self, include_friends, include_modders)
    local player_list = {}
    for _, player_id in pairs(players.list(include_self, true, true)) do
        if (include_friends or not menu.get_tags_string(player_id):find("F"))
        and (include_modders or not menu.is_marked_as_modder(player_id)) then
            table.insert(player_list, Ryan.Player.ById(player_id))
        end
    end
    return player_list
end

-- Create the player object or update it.
Ryan.Player.CreateOrUpdate = function(player_id)
    -- Create the player object if it doesn't exist.
    local created = false
    if Ryan.Player.Instances[player_id] == nil then
        Ryan.Player.Instances[player_id] = {id = player_id}
        created = true
    end

    local player = Ryan.Player.Instances[player_id]
    if created then
        ...
        player.supercrash = function(block_syncs)
            util.toast("Crashing a player...")
            ...
            if block_syncs then player.block_syncs(crash)
            else crash() end
        end

        -- Perform an action while blocking syncs to the player.
        player.block_syncs = function(action)
        ...
        player.spam_sms_and_block_joins = function(block_joins, sms_message, action)
            ...
            if sms_message ~= "" and sms_message ~= " " then
                ...
                Ryan.Player.SpamSMS(player.id, sms_message, 6000)
            end
            action()
